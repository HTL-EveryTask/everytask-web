...
const startTime = ref(new Date().toJSON().slice(0, 16));
const endTime = ref(new Date().toJSON().slice(0, 16));
const assigned = ref([]);
...
const v$ = useVuelidate(
  rules,
  { title, description, startTime, assigned, tags },
  { $autoDirty: true }
);
...
async function onSubmit() {
  ...
  title.value = "";
  description.value = "";
  startTime.value = new Date().toJSON().slice(0, 16);
  endTime.value = new Date().toJSON().slice(0, 16);
  assigned.value = [];
  tags.value = [];
  ...
  >
    ...
    <form @submit.prevent="onSubmit">
          ...
          <div class="w-full p-4">
            ...
            <div class="flex gap-2">
              <InputField id="due" :validation="v$.due" label="Due">
                <input id="due" v-model="startTime" type="datetime-local" />
              </InputField>
              <InputField id="due" :validation="v$.due" label="Due">
                <input id="due" v-model="endTime" type="datetime-local" />
              </InputField>
            </div>
      ...
      <div class="flex items-center rounded-full p-2 gap-4">
        ...
        <div class="flex-1 flex items-center relative max-w-[18em]">
          <input
            v-model="title"
